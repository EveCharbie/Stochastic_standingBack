...
def custom_dynamics(
    ...
    for i in range(nlp.model.nb_random):
        ...
        # Motor noise
        motor_noise_this_time = motor_noise[:, i]
        motor_noise_this_time[1] = 0  # No noise on the eyes
...
def prepare_socp_FEEDFORWARD(
    ...
    """
    Sensory inputs:
    - proprioceptive: joint angles and velocities (4+4), no proprioception on eyes
    ...
    for i in range(n_joints):
        if i != 1:  # No frictions on the eyes
    ...
    objective_functions.add(ObjectiveFcn.Lagrange.MINIMIZE_CONTROL, key="k", weight=1, quadratic=True, derivative=True)
