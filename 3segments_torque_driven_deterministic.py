...
def prepare_ocp(
    ...
    objective_functions.add(ObjectiveFcn.Mayer.MINIMIZE_COM_VELOCITY, node=Node.END, weight=-1e3*2, quadratic=False,
                            axes=Axis.Z)  # Temporary while in 1 phase ?
    ...
    q_min = np.ones((n_q, n_shooting+1)) * -2*np.pi
    q_max = np.ones((n_q, n_shooting+1)) * 2*np.pi
    q_min[:, 0] = [-0.1939-0.3, -0.0388-0.3, -1.0471-0.3, 1.4861-0.3, 0.4327-0.3]  # Initial position approx from bioviz
    q_max[:, 0] = [-0.1939+0.3, -0.0388+0.3, -1.0471+0.3, 1.4861+0.3, 0.4327+0.3]
    q_min[:n_trans, 1:] = -1
    q_max[:n_trans, 1:] = 1
    q_min[2, 1:] = -np.pi/2
    q_max[2, 1:] = 0
    ...
    qdot_min = np.ones((n_qdot, n_shooting+1)) * -100
    qdot_max = np.ones((n_qdot, n_shooting+1)) * 100
    qdot_min[:, 0] = 0
    qdot_max[:, 0] = 0
    qdot_min[:, 1:] = -10*np.pi
    qdot_max[:, 1:] = 10*np.pi
...
def main():
    ...
    biorbd_model_path = "3segments_2(3)contact.bioMod"
    save_path = f"{biorbd_model_path[:-7]}_torque_driven_1phase_socp.pkl"
